...
exports.scrapeBook = async (url) => {
  try {
    ...
    let coverImage = $('#imgBlkFront').attr('src') || $('#ebooksImgBlkFront').attr('src') || $('#landingImage').attr('src');
    if (coverImage && coverImage.includes('data:image')) {
        const dynamicImages = $('#imgBlkFront').attr('data-a-dynamic-image') || 
    ...
    $('#detailBullets_feature_div ul li, #productDetailsTable tr, #detailBulletsWrapper_feature_div ul li, #detail-bullets table tr').each((i, el) => {
