...
const useBookFormLogic = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const genresList = Array.isArray(data.genres)
          ? data.genres.map((g) => ({ value: g.name, label: g.name }))
          : [];
        const authorsList = Array.isArray(data.authors)
          ? data.authors.map((a) => ({ value: a.name, label: a.name }))
          : [];
        const translatorsList = Array.isArray(data.translators)
          ? data.translators.map((t) => ({ value: t.name, label: t.name }))
          : [];
        ...
        if (isEditMode) {
          ...
          setFormData({
            ...
            selectedGenre:
              bookGenres.length > 0
                ? { value: bookGenres[0].name, label: bookGenres[0].name }
                : null,
            selectedSubgenre:
              bookSubgenres.length > 0
                ? { value: bookSubgenres[0].name, label: bookSubgenres[0].name }
                : null
          });
        ...
        console.error('Erro de fetchInitialData:', error);
        setFeedback({
          type: 'error',
          message: 'Erro ao carregar os dados. Verifique a sua conexão.'
        });
      }
    };
    fetchInitialData();
  ...
  const handleCropComplete = (file, url) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setFeedback({
        type: 'error',
        message:
          'A imagem original é muito complexa e, mesmo após a compressão, excedeu 2MB. Por favor, tente uma foto mais leve.'
      });
    ...
    try {
      ...
      if (!fetchedData) {
        try {
          ...
          console.warn('Google Books falhou.', e);
        }
      }
      ...
      if (!fetchedData) {
        try {
          ...
          console.warn('Open Library falhou.', e);
        }
      }
      ...
      if (fetchedData) {
        ...
        setFormData((prev) => ({
          ...prev,
          isbn: formatIsbnInput(cleanIsbn),
        ...
        if (fetchedData.coverUrl && !coverFile) {
          try {
            ...
            console.warn('Falha no download da capa da API', err);
            setPreviewUrl(fetchedData.coverUrl);
          }
        }
      ...
      console.error('Erro Geral de Busca ISBN:', error);
      setFeedback({
        type: 'error',
        message: 'Erro ao buscar as informações. Verifique sua conexão.'
      });
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    payloadForm.append(
      'authors',
      JSON.stringify(formData.authors ? formData.authors.map((a) => a.value) : [])
    );
    payloadForm.append(
      'translators',
      JSON.stringify(formData.translators ? formData.translators.map((t) => t.value) : [])
    );
    payloadForm.append(
      'tags',
      JSON.stringify(
    ...
    payloadForm.append(
      'genres',
      JSON.stringify(formData.selectedGenre ? [formData.selectedGenre.value] : [])
    );
    payloadForm.append(
      'subgenres',
      JSON.stringify(formData.selectedSubgenre ? [formData.selectedSubgenre.value] : [])
    );
